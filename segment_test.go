...
func TestSegment_Write_FULL2(t *testing.T) {
	...
	pos1, err := seg.Write(val)
	require.Nil(t, err)
	require.Equal(t, pos1.BlockNumber, uint32(0))
	require.Equal(t, pos1.ChunkOffset, int64(0))
	val1, err := seg.Read(pos1.BlockNumber, pos1.ChunkOffset)
	require.Nil(t, err)
	require.Equal(t, val, val1)
	...
	pos2, err := seg.Write(val)
	require.Nil(t, err)
	require.Equal(t, pos2.BlockNumber, uint32(1))
...
func TestSegment_Write_Padding(t *testing.T) {
	...
	pos1, err := seg.Write(val)
	require.Nil(t, err)
	require.Equal(t, pos1.BlockNumber, uint32(1))
	require.Equal(t, pos1.ChunkOffset, int64(0))
	val1, err := seg.Read(pos1.BlockNumber, pos1.ChunkOffset)
	require.Nil(t, err)
	require.Equal(t, val, val1)
...
func TestSegment_Reader_FULL(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t, bytes1, val)
	require.Equal(t, pos1, rpos1)

	val, rpos2, err := reader.Next()
	require.Nil(t, err)
	require.Equal(t, bytes1, val)
	require.Equal(t, pos2, rpos2)
...
func TestSegment_Reader_Padding(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t, bytes1, val)
	require.Equal(t, pos1.SegmentId, rpos1.SegmentId)
	require.Equal(t, pos1.BlockNumber, rpos1.BlockNumber)
	require.Equal(t, pos1.ChunkOffset, rpos1.ChunkOffset)

	val, rpos2, err := reader.Next()
	require.Nil(t, err)
	require.Equal(t, bytes1, val)
	require.Equal(t, pos2.SegmentId, rpos2.SegmentId)
	require.Equal(t, pos2.BlockNumber, rpos2.BlockNumber)
	require.Equal(t, pos2.ChunkOffset, rpos2.ChunkOffset)
...
func TestSegment_Reader_NOT_FULL(t *testing.T) {
	...
	require.Equal(t, pos1, rpos1)
	require.Equal(t, pos2, rpos2)
	require.Equal(t, pos3, rpos3)
	require.Equal(t, pos4, rpos4)
}
...
func TestSegment_Reader_ManyChunks_FULL(t *testing.T) {
	...
	for {
		...
		require.Equal(t, positions[i].SegmentId, pos.SegmentId)
		require.Equal(t, positions[i].BlockNumber, pos.BlockNumber)
		require.Equal(t, positions[i].ChunkOffset, pos.ChunkOffset)

		i++
	}
	require.Equal(t, 1000000, len(values))
}
...
func TestChunkPosition_Encode(t *testing.T) {
	validate := func(pos *ChunkPosition) {
		...
		require.Equal(t, pos, decRes)
	}

	validate(&ChunkPosition{1, 2, 3, 100})
	validate(&ChunkPosition{0, 0, 0, 0})
	validate(&ChunkPosition{math.MaxUint32, math.MaxInt64, math.MaxUint32, math.MaxUint32})
}
...
func TestChunkPosition_EncodeFixedSize(t *testing.T) {
	validate := func(pos *ChunkPosition) {
		...
		require.NotNil(t, res)
		require.Equal(t, binary.MaxVarintLen32*3+binary.MaxVarintLen64, len(res))
		decRes := DecodeChunkPosition(res)
		require.Equal(t, pos, decRes)
	}
